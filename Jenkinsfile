...
rocHPCGMPICI:
{
    ...
    def rochpcg = new rocProject('rochpcgx2')
    // customize for project
    rochpcg.paths.build_command = './install.sh -t --with-openmp=OFF'

    // Define test architectures, optional rocm version argument is available
    def nodes = new dockerNodes(['ucx'], rochpcg)
    ...
    def testCommand =
    {
        ...
        if(auxiliary.isJobStartedByTimer())
        {
          command = """#!/usr/bin/env bash
                set -x
                cd ${project.paths.project_build_prefix}/build/release/tests
                LD_LIBRARY_PATH=/opt/rocm/hcc/lib mpirun -np 2 --allow-run-as-root ./rochpcg-test --gtest_output=xml --gtest_color=yes #--gtest_filter=*nightly*-*known_bug* #--gtest_filter=*nightly*
            """
        }
        else
        {
          command = """#!/usr/bin/env bash
                set -x
                cd ${project.paths.project_build_prefix}/build/release/tests
                LD_LIBRARY_PATH=/opt/rocm/hcc/lib mpirun -np 2 --allow-run-as-root ./rochpcg-test --gtest_output=xml --gtest_color=yes #--gtest_filter=*quick*:*pre_checkin*-*known_bug* #--gtest_filter=*checkin*
            """
        }
