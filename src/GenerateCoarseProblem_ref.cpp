...
void GenerateCoarseProblem_ref(const SparseMatrix & Af) {
  ...
  GenerateProblem_ref(*Af.Ac, 0, 0, 0);
  SetupHalo_ref(*Af.Ac);
  InitializeVector(*Af.mgData->rc, Af.Ac->localNumberOfRows);
  InitializeVector(*Af.mgData->xc, Af.Ac->localNumberOfColumns);
  InitializeVector(*Af.mgData->Axf, Af.localNumberOfColumns);
  InitializeMGData(f2cOperator, *Af.mgData);
